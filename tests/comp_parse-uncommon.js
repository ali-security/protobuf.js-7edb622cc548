...
tape.test("parser nesting at the default limit", function(test) {
    var messages = "";
    var options = "message M { option foo = {";
    for (var i = 0; i < 20000; ++i) {
        messages += "message M" + i + " {";
        options += "a {";
    }
    for (i = 0; i < 20000; ++i) {
        messages += "}";
    ...
    test.throws(function() {
        protobuf.parse(messages);
    }, /max depth exceeded/, "should reject messages nested beyond the default recursion limit");
    test.throws(function() {
        protobuf.parse(options);
    }, /max depth exceeded/, "should reject aggregate options nested beyond the default recursion limit");
